...
local function setGhostTarget(ghost)
    local candidates = {}
    ...
    if (maze[ghost.yTile + 1][ghost.xTile] == 1 or maze[ghost.yTile + 1][ghost.xTile] == 2 or (maze[ghost.yTile + 1][ghost.xTile] == 3 and ghost.dead)) and ghost.direction ~= "up" then
        candidates[#candidates + 1] = {ghost.xTile, ghost.yTile+1}
    end
    ...
    local function setDirection(from, to, prefix)
        ...
        if dir then
            from.report = "Entered tile "..ghost.xTile.."/"..ghost.yTile ..", " .. prefix .. msg
            from.nextDir = dir
        end
    end
            ...
            if (timer.restart and timer.restart > 0) then
                ...
                if timer.restart <= 0 then
                    ...
                    for i, ghost in ipairs(ghosts) do
                        ...
                        ghost.leavingTimer = Maze.getLeavingTimer(gameState.level, ghost.name)
                    end
                    timer.startup = 2
                    timer.fruit = false
                    for i, ghost in ipairs(ghosts) do
            ...
            for i, ghost in ipairs(ghosts) do
                ...
                if ghost.leaving then
                    if not ghost.leavingTimer then
                        ...
                        ghost.leavingTimer = ghost.leavingTimer - dt
                        if ghost.leavingTimer <= 0 then ghost.leavingTimer = false end
                    end
                else
...
function love.draw()
    ...
    if (gameState.mode == "attract") then
        ...
        local creditText = "Pinbac-Man v0.1 by Pinback - A Jolt Country Games development"
        local creditFont = love.graphics.newFont(PIXEL_SIZE * 5)
        love.graphics.setFont(creditFont)
        local creditWidth = creditFont:getWidth(creditText);
        love.graphics.setColor(.4, 1, .4)
        love.graphics.print(creditText, windowWidth / 2 - creditWidth / 2, windowHeight - PIXEL_SIZE * 10)
    else
        ...
        love.graphics.print(Maze.getFruit(gameState.level).name, TILE_SIZE * (#maze[1]/1.7), (#maze - 2) * TILE_SIZE - (PIXEL_SIZE * 4))

...
function renderer.drawGhosts(ghosts, config)
    ...
    for i,ghost in ipairs(ghosts) do
        ...
        if (gameState.ateGhost ~= ghost.name) then
            ...
            if (ghost.leavingTimer) then
                -- Draw spinning eyes while ghost is waiting to leave the pen
                local t = love.timer.getTime()
                local spinRadius = tileSize * 0.22
                local angle = t * 4 -- spin speed, adjust as desired
                -- Both eyes spin together; same angle
                for eye = 1, 2 do
                    local ex = ghost.x * pixelSize + (eye == 1 and -tileSize/3 or tileSize/3)
                    local ey = ghost.y * pixelSize - (tileSize/3)
                    -- Both pupils have the same angle offset for perfect sync
                    local dx = math.cos(angle) * spinRadius
                    local dy = math.sin(angle) * spinRadius
                    love.graphics.circle("fill", ex + dx, ey + dy, tileSize * .15)
                end
